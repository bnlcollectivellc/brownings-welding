    ...
    <>
      <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32 w-full">
          ...
          {/* Top Section - Logo, tagline, CTA (left-aligned on desktop, centered on mobile) */}
          <div className="text-center lg:text-left lg:max-w-xl mb-10">
            {/* Logo */}
            <div className="mb-6">
              <Image
                ...
                width={500}
                height={167}
                className="mx-auto lg:mx-0 h-auto w-[260px] md:w-[400px] lg:w-[450px]"
                priority
              />
            </div>
          ...
          {/* Divider */}
          <div className="flex items-center justify-center lg:justify-end gap-4 mb-10">
            <div className="h-px bg-white/30 w-24"></div>
            <span className="text-white/60 text-sm">or choose how to start</span>
            <div className="h-px bg-white/30 w-24"></div>
          </div>

          {/* Bottom Section - Upload zone and cards (right-aligned on desktop, centered on mobile) */}
          <div className="lg:ml-auto lg:max-w-2xl">
            {/* Upload Zone */}
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 mb-8 border border-white/20">
              <div
                className={`upload-zone border-2 border-dashed rounded-xl p-6 text-center cursor-pointer ${
                  isDragging
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              {entryCards.map((card) => (
                <button
                  ...
                  className="entry-card bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-3 md:p-5 text-left hover:bg-white/15 group"
                >
                  <card.icon className="text-browning-red mb-2 md:mb-3" size={24} />
                  <h3 className="text-white font-semibold text-sm md:text-base mb-1 md:mb-2">{card.title}</h3>
                  <p className="text-gray-400 text-xs md:text-sm mb-2 md:mb-3 line-clamp-2">{card.description}</p>
                  <div className="flex items-center text-browning-red font-medium text-xs md:text-sm group-hover:gap-2 transition-all">
            ...
            {/* Disclaimer */}
            <p className="text-center lg:text-right text-gray-500 text-xs mt-6">
